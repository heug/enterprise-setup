resource "aws_launch_configuration" "mac_lc" {
  # 4x or 8x are best
...
resource "aws_autoscaling_group" "mac_asg" {
  name = "${var.prefix}_${var.name}_asg"

  vpc_zone_identifier  = ["${var.aws_subnet_id}"]
  launch_configuration = "${aws_launch_configuration.mac_lc.name}"
...
resource "aws_autoscaling_lifecycle_hook" "mac_shutdown_hook" {
  name                    = "${var.prefix}_${var.name}_shutdown_hook"
  autoscaling_group_name  = "${aws_autoscaling_group.mac_asg.name}"
